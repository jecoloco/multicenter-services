...
export async function customer(ctx: Context, next: () => Promise<any>) {
  console.log(
    '============= INICIO REQUEST MULTICARD CUSTOMER ===================='
  )
  ...
  try {
    ...
    if (!token || !document) {
      ctx.status = 400
      ctx.body = { error: 'Faltan parámetros token o document' }

      return
    }

    const response = await ctx.clients.multicardCustomerClient.getCustomerByDoc(
      token,
      document
    )
  ...
  console.log(
    '============== FIN REQUEST MULTICARD CUSTOMER ====================='
  )
  await next()
}
